...
describe('CLI vault flow', () => {
  it('supports init/add/get/list in JSON mode', () => {
  ...
  vault_decrypt: (entryJson) => {
    ...
    const add = execSync(`${envPrefix} npx tsx src/infra/cli/index.ts vault add --key SHARED_LLM_API_KEY --value sk-test --json`, {
      encoding: 'utf8',
    });
